...
const MOON_TEXTURE = '/images/hyperreal-moon.png'
...
function Moon({ onSelect, selected }: { onSelect: (id: string) => void; selected: string | null }) {
  return <div className="moon-stage" aria-label="Interactive lunar surface map">
    <div className="orbital-ring ring-one" /><div className="orbital-ring ring-two" /><div className="moon-glow" />
    <div className="moon" style={{ backgroundImage: `url(${MOON_TEXTURE})` }}>
      <div className="moon-shade" /><div className="moon-specular" />
    ...
    <span className="orbital-readout readout-top">LAT 28.6139° N</span><span className="orbital-readout readout-bottom">LON 77.2090° E</span>
  </div>
}
...
export default function Page() {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => { if (event.key === 'Enter' && !booted) setBooted(true); if (event.key === 'Escape') { setSelected(null); setShowFuture(false) } }
  ...
  return <main className="site-shell" style={{ '--px': `${parallax.x}px`, '--py': `${parallax.y}px` } as React.CSSProperties}>
    ...
    <section className="hero"><div className="hero-copy"><p className="eyebrow">SUSMIT BHAR / ENGINEER + BUILDER</p><h1>Building at the<br /><em>edge of possible.</em></h1><p className="hero-intro">Electrical engineering, software, and a steady pull toward space. Explore the surface to inspect the systems I&apos;m learning to shape.</p><div className="hero-meta"><span>06 ACTIVE REGIONS</span><span>·</span><span>FIELD LOG 2026</span></div></div><Moon onSelect={(id) => { setSelected(id); setShowFuture(false) }} selected={selected} /></section>
    ...
    <div className="telemetry"><span><b>01</b> / 06</span><span className="telemetry-line" /><span>MOVE TO EXPLORE <span className="mouse-icon">↓</span></span></div>
